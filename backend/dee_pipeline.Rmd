...
lapply( unalloc, function(x) {
  file.move( x ,  paste(DATAWD,"/trash/",basename(x),sep="") )
} )
...
mydates <- apply(X=(expand.grid(sprintf('%0.2d', 1:31),sprintf('%0.2d', 1:12))),MAR=1,FUN=function(x) {paste(x[1],x[2],sep="-" ) }  )

pysrares2 <- lapply(mydates,function(mydate) {
  pysra_cmd <- paste("~/miniconda3/bin/python -m pysradb search -d geo -m 10000 -O ", species_name, " -C TRANSCRIPTOMIC -D ",mydate,"-2026:",mydate,"-2026 --detailed ",sep="")
  pysrares <- system(pysra_cmd,intern=TRUE)
  pysrares2 <- do.call(rbind,strsplit(pysrares,"\t"))
  colnames(pysrares2) <- pysrares2[1,]
  pysrares2 <- pysrares2[-1,]
  Sys.sleep(5)
  return(pysrares2)
})

# remove elements with no data
pysrares2 <-  pysrares2[which(unlist(lapply(pysrares2,function(x) { class(x)[1] } ))=="matrix")]

GEOFILENAME = paste(SRADBWD,"/",org,"_geo_2026.csv",sep="")

# Grab the GSE and GSM data
GSE <- unlist(lapply(pysrares2,function(x) {x[,"study_alias"] } ))
GSM <- unlist(lapply(pysrares2,function(x) {x[,"sample_alias"] } ))
gse <- unique(data.frame(GSE,GSM))
colnames(gse) <- c("GEO_series","GEO_sample")
write.table(gse,GEOFILENAME,sep=",")

#pysra_cmd <- paste("~/miniconda3/bin/python -m pysradb search -d geo -m 10000 -O ", species_name, " -C TRANSCRIPTOMIC -D 01-01-2026:31-12-2026 --detailed ")
#pysrares <- system(pysra_cmd,intern=TRUE)
#pysrares2 <- do.call(rbind,strsplit(pysrares,"\t"))
#colnames(pysrares2) <- pysrares2[1,]
#pysrares2 <- pysrares2[-1,]
#GEOFILENAME = paste(SRADBWD,"/",org,"_geo_2026.csv",sep="")
#GSE <- pysrares2[,"study_alias"]
#GSM <- pysrares2[,"sample_alias"]
gse <- unique(data.frame(GSE,GSM))
colnames(gse) <- c("GEO_series","GEO_sample")
write.table(gse,GEOFILENAME,sep=",")
