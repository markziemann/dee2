...
Any data from earlier years has been previously downloaded so we only need to download current year data.

```{r,geoquery1}

geofiles <- list.files(paste(SRADBWD,"/",org,"_geo",sep=""),recursive=TRUE,full.names=TRUE)

geo_metadata_lines <-  lapply(geofiles, function(x) {
  readLines(x)
})

geofiles <- geofiles[which(unlist(lapply(geo_metadata_lines,length))>1)]

geo_metadata <-  lapply(geofiles, function(x) {
  read.csv(x)
})

GSM <- unlist(lapply(geo_metadata,function(x) { x[,"sample_alias"] } ) )

# Sometimes the study alias colname is not there !?
GSE <- unlist(lapply(geo_metadata,function(x) {
  if ( length(which(colnames(x)=="study_alias")) == 1 ) {
    x[,"study_alias"]
  }
  else {
    x[,"submission_alias"]
...
gse <- unique(data.frame(GSE,GSM))
colnames(gse) <- c("GEO_series","GEO_sample")

GEOFILENAMENEW = paste(SRADBWD,"/",org,"_geo_new.csv",sep="")
GEOFILENAMEOLD = paste(SRADBWD,"/",org,"_geo_old.csv",sep="")
if (file.exists(GEOFILENAMENEW)) {
  renameFile(GEOFILENAMENEW,GEOFILENAMEOLD,overwrite=TRUE)
}
write.table(gse,GEOFILENAMENEW,sep=",")
...
resx <- merge(md_df,gse,by.x="sample_alias",by.y="GEO_sample",all.x=TRUE)
