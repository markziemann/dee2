...
cut -f-3 TxInfo.tsv | tr '\t' '_' | paste - TxCountMatrix.tsv  | cut -f1,3- | cut -d '_' -f2- > tmp.tsv

cat <<EOF > tmp.R
x<-read.table("tmp.tsv",header=T)
y<-aggregate(. ~ GeneID_GeneSymbol,x,sum)
write.table(y,file="Tx2GeneCountMatrix.tsv",quote=F,sep="\t",row.names = F)
EOF
Rscript tmp.R
