...
#echo "TOKEN $TOKEN"
#echo "<br>"
curl -b cookies.txt -v -L 'https://degust.erc.monash.edu/upload' -F "authenticity_token=$TOKEN" -F "filename=@/$PWD/TxCountMatrix.tsv" 2> tmp \
| grep -v 'front-loader.gif'

URL=$(grep -m1 'https://degust.erc.monash.edu/degust/compare.html?code=' tmp | cut -d ' ' -f3)
echo "Sending your TxCountMatrix.tsv to Degust."
